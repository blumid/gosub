...
func initialCommands(outdir string, wordlist string, resolver string) map[int]string {
	commands := map[int]string{
		...
		// add new things to final && delete wl
		8: "cat " + outdir + "/%[1]s" + "/step1 | anew -q " + outdir + "/%[1]s" + "/final && rm -f " + outdir + "/%[1]s" + "/wl",

		// gotator - depth 2
		9: "gotator -silent -sub " + outdir + "/%[1]s" + "/final -depth 2 -mindup > " + outdir + "/%[1]s" + "/gotator1",
		...
		// seperate hidden and final, delete round2
		14: "cat " + outdir + "/%[1]s" + "/round2 | anew " + outdir + "/%[1]s" + "/final",
		// httpx - json file
